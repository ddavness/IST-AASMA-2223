...
def fixDirection(action: int, direction_ptr: int, observation: dict, agent_i: int):
...
def food(percentage, observation, pos=None):
    closestFood = []
    distances = []
...
def get_valid_actions(direction_ptr: int, observation: dict, pos: tuple):
    ...
    for action in Action:
        aux = sum(pos, observation["directions"][fixDirection(action.value, direction_ptr, observation, pos)])
...
def get_neighbours(direction_ptr: int, observation: dict, pos: tuple):
    ...
    for action in actionValid:
        neighbours += [sum(pos, observation["directions"][fixDirection(action, direction_ptr, observation, observation["agents"][observation["self"]][0] )])]
